...
def train_data(queue, birds, level=3000):
    ...
    for listi in queue:
        for idnew in listi:
            if dict_id_pred[idnew][0] > level and idnew in titleContianUser:
...
def predict_data(position, predict_example, level=3000, true_predict = []):
    ...
    for i in predict_example:
        if dict_id_pred[i][0] > level and i in titleContianUser:
            new_temp = classify_dict[i].reshape(1, 15)
            new_temp = np.zeros(shape=(1, 15))
            for uid in titleContianUser[i]:
                new_temp += userlike[uid]
            new_temp = new_temp/len(titleContianUser[i])
            new_temp2 = classify_dict[i].reshape(1, 15)
            pred_new = dict_id_pred[i][0] + np.dot(np.dot(new_temp, position), new_temp2.T)
