...
titleContianUser = np.load("titleContianUser.dict.npz")['arr_0'][()]
train_day15 = []
...
def fitFunc(position, allexample):
    ...
    basesum = 0.0
    for i in allexample:
        new_temp = np.zeros(shape=(1, 15))
        for uid in titleContianUser[i[0]]:
            new_temp += userlike[uid]
        new_temp = new_temp/len(titleContianUser[i[0]])
        new_temp2 = classify_dict[i[0]].reshape(1, 15)
        pred_new = i[1] + np.dot(np.dot(new_temp, position), new_temp2.T)
...
class PSO:
    ...
    def __init__(self, fitFunc, birdNum, c1, c2, solutionSpace, extend, birds):
        ...
        self.w = 0.9
        self.extend = extend
        self.solutionmax = 500
        self.solutionmin = -500
        #self.solutionmax = 10*solutionSpace
        #self.solutionmin = -10*solutionSpace
        if birds == None:
            ...
            for i in range(birdi.position.shape[0]):
                for j in range(birdi.position.shape[1]):
                    if birdi.position[i][j] > self.solutionmax:
                        birdi.position[i][j] = self.solutionmax*0.4 + self.solutionmax*random.random()*0.6
                    if birdi.position[i][j] < self.solutionmin:
                        birdi.position[i][j] = self.solutionmin*0.4 + self.solutionmin*random.random()*0.6
            # 跟新适应度
