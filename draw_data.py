...
class draw_data():
    ...
    def title_comment_time(self, titleid, begin_time, last_time):
        return self.db.CommentItem.aggregate([
                    {"$match": {"time": { "$gt": begin_time, "$lt": last_time }, "title_id" : titleid }},
                    {"$group": {"_id": "$title_id", "count":{"$sum": 1}}}
                ])

    def return_user(self, num):
        return self.db.CommentItem.aggregate([
    ...
    def get_title_data(self, begin_time, last_time, limit_num):
        ...
        ).sort([("title_time", pymongo.ASCENDING)]).limit(limit_num)
    ...
    def title_comment_aggregate(self, titleid):
        return self.db.CommentItem.aggregate([
            {"$match": {"title_id" : titleid}},           
            {"$group": {"_id": "$user", "count":{"$sum": 1}}} 
            ])
